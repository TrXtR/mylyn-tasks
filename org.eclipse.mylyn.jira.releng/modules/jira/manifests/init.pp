class jira {
  ...
  /* Common requirements for all jira instances */
  ...
  /* jira automatically provisions during install "python-genshi" */
  ...
  exec { "prepare jira":
    command => "echo jira pre-requisites are installed",
    require => Package[$requirements],
  }
  
  file { "/usr/lib/cgi-bin/services":		
    source  => "puppet:///modules/jira/services.cgi",		
    mode => 755,		
    require => Package[$requirements],
  }
